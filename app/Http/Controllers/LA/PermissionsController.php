...
class PermissionsController extends Controller
{
	...
	public function index()
	{
		...
		$user_roles = DB::table('roles')->select('*')->where('id', '!=' , 1)->whereNull('deleted_at')->get();
		$role_modules = DB::table('role_module')->select('*')->get();
	...
	public function store(Request $request)
	{
      	...
      	$i=0;
		 	foreach($_POST['data'][$role] as $data){
			  $module_id=!empty($_POST['data'][$role][$i]['module_id'])?$_POST['data'][$role][$i]['module_id']:'';	
				$role_id=!empty($_POST['data'][$role][$i]['role_id'])?$_POST['data'][$role][$i]['role_id']:'';
				$datas['acc_view']=!empty($_POST['data'][$role][$i]['acc_view'])?$_POST['data'][$role][$i]['acc_view']:0;
				$datas['acc_create']=!empty($_POST['data'][$role][$i]['acc_create'])?$_POST['data'][$role][$i]['acc_create']:0;
				$datas['acc_edit']=!empty($_POST['data'][$role][$i]['acc_edit'])?$_POST['data'][$role][$i]['acc_edit']:0;
				$datas['acc_delete']=!empty($_POST['data'][$role][$i]['acc_delete'])?$_POST['data'][$role][$i]['acc_delete']:0;	
