		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) {
				...
				if($fields_popup[$col] != null && $fields_popup[$col]->field_type_str == "Image") {
					$tmp = $data->data[$i][$j];
					if(count($tmp) > 0) {
						$upload = \App\Models\Upload::findFile($tmp);								
						if(isset($upload->id)) {
							$data->data[$i][$j] = '<img style="width:50px;height:50px"src="'.url("files/".$upload->hash.DIRECTORY_SEPARATOR.$upload->name).'">';
						} else {
							$data->data[$i][$j] = "No Image";
						}
					} else {
						$data->data[$i][$j] = "No Image";
					}
