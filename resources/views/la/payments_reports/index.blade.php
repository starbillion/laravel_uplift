...
@section("contentheader_title", "Payments Reports")
@section("contentheader_description", "Payments Reports")
@section("section", "Payments Reports")
@section("sub_section", "Reports")
@section("htmlheader_title", "Payments Reports")
...
<div class="box box-success">
	...
	{!! Form::open(['action' => 'LA\Payments_ReportsController@store', 'id' => 'ride-form']) !!}
		<div class="pull-left col-lx-12">
		  ...
		  <div class="pull-left col-lg-2 mr_10">
			...
			<div class="pull-left col-lg-3 mr_10"> 
			  <select name="reimb_status" class="form-control select2" >
			  	 <option value="" @if($reimb_status == '') {{'selected="selected"'}} @endif>Select Status</option>
			  	 <option value="1" @if($reimb_status == '1') {{'selected="selected"'}} @endif>Paid</option>
			  	 <option value="0" @if($reimb_status == '0') {{'selected="selected"'}} @endif>Unpaid</option>
			  </select>
			</div>
			<!--div class="pull-left col-lg-3 mr_10">
	...
	{!! Form::open(['action' => 'LA\Payments_ReportsController@store', 'id' => 'ride-form']) !!}
	 ...
	 <input type="hidden" name="driver_filter" value="{{!empty($driver_filter)?$driver_filter:''}}" >	 
	 <input type="hidden" name="reimb_status" value="{{!empty($reimb_status)?$reimb_status:''}}" >	 
	  ...
	  @if(!empty($rideDatass)) 
		...
		<tbody>
		 {{--*/ $k=1; /*--}}
				 ...
				 <tr>                                     
           <td>   
           <?php if($rideData->reimb_status == 0) { ?>
				   <input type="checkbox" class="checkboxSelected" value="<?= $rideData->payment_id;?>" name="payments[]" onClick="checkboxPayment()" data-amount="<?php echo LAHelper::earningAmount($rideData->paid_amount,$rideData->driver_id); ?> " data-feeamount="<?php echo LAHelper::subChargesFee($rideData->paid_amount,$rideData->driver_id); ?> ">
				   <?php } ?>
					 </td>
					 ...
					 <td><?php echo '£ '.LAHelper::earningAmount($rideData->paid_amount,$rideData->driver_id); ?></td>
					 <td><?php echo LAHelper::subCharges($rideData->driver_id); ?></td>	
					 <td><?php echo '£ '.LAHelper::subChargesFee($rideData->paid_amount,$rideData->driver_id); ?></td>
					 <td> <?php if($rideData->reimb_status == 0){        		
...
$(function () {
    ...
    columnDefs: [ { orderable: false, targets: [0] }],
        ajax: "{{ url(config('laraadmin.adminRoute') . '/payments_report_dt_ajax') }}",
		language: {
			lengthMenu: "_MENU_",
			search: "_INPUT_",
...
$(function () {
    ...
    $('#example1_length span').remove('.btnClass');
      ...
      $('#example1_length span').remove('.btnClass');
		...
		$('#amt').html('');
    $('#amt').html('Toatal Amount : £ '+amt.toFixed(2)+' <span style="margin-left:15px">  Fee Amount : £ '+feeamt.toFixed(2)+'</span>');
	}); 
